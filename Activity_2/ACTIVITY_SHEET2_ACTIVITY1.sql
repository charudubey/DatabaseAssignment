...
CREATE TABLE resources.location(location_id INT PRIMARY KEY, location_name VARCHAR(200));



INSERT INTO `resources`.`location`
            (`location_id`,
             `location_name`)
VALUES (1,
        "Indore");
...
CREATE TABLE resource_hours(resource_hrs_id INT PRIMARY KEY, resource_id INT , hours INT, week_end_date DATE, FOREIGN KEY (resource_id) REFERENCES resource(id));


INSERT INTO `resources`.`resource_hours`
            (`resource_hrs_id`,
             `resource_id`,
             `hours`,
             `week_end_date`)
